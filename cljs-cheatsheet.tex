...
With Method(s):& \cmd{(deftype Pair [h t]}\\ 
               & \cmd{  Object (toString [] ...))} \\
\end{tabularx}
...
Method:& \cmd{(defmethod my-mm :dispatch-value}\\ 
       & \cmd{  [args] ...)}\\
\end{tabularx}
...
\begin{tabularx}{\hsize}{lX}
Method Call: & \cmd{(.method obj args) |} \\
  & \cmd{(. obj (method args))} \\
...
Create JS Array: & \cmd{(array items) | }\\
                 & \cmd{(make-array size)} \\
Transform JS: & \cmd{(js->clj js-val)} \\
Transform CLJ: & \cmd{(clj->js clj-val)} \\
\end{tabularx}
